...
def proc_monthly():
    # Get shapefile
    gdf = gpd.read_file("data/shapefiles/willamette_valley.shp")
    
    # Build dataframe to process years and variables
    year_ = np.arange(1895, 2021, 1).astype(str)
    var_ = ["tmax", "tmin", "ppt"]
    ...
    outdat.to_csv("data/PRISM_monthly_1895-2020.csv", index=False)
...
def proc_daily():
    ...
    # Build dataframe to process years and variables
    year_ = np.arange(1980, 2021, 1).astype(str)
    var_ = ["tmax", "tmin", "ppt"]
    ...
    # Concat and save
    outdat = pd.concat(df_list)
    outdat.to_csv("data/PRISM_daily_1981-2020.csv", index=False)
